...
def TwoMA(data , period , small_MA_period , long_MA_period , init_money , invest_percent , print_graph = False) :
    ...
    if print_graph:
        ...
        # print(all_data)
        draw_Candlestick(all_data , draw_TwoMA = True , save_path = f"TwoMA_Candlestick_{10-period}-{period}")
        draw_accumulated_capital(all_data , save_path = f"TwoMA_Money_{10-period}-{period}_{round(IRR,3)}")
        draw_combin_fig(all_data , draw_TwoMA = True , save_path = f"TwoMA_combin_{10-period}-{period}_{round(IRR,3)}")
